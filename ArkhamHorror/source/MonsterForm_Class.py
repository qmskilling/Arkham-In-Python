...
"""
Monster form submission v 1.1b - Class Only
This code will create a GUI to submit a new monster. Class will be callable from external code.

In this version (v1.1b):
A GUI prompts user for monster information, including images to be used for monster tokens.
...
class Window(QDialog):
    ...
    def initWindow(self):
        ...
        self.DB_connection()
        vbox.addWidget(self.db_groupBox)
    ...
    def DB_connection(self):
        self.db_groupBox = QGroupBox('Database Connection')
        self.db_vBox = QVBoxLayout()

        self.db_conn_btn = QPushButton("Open Database")
    ...
    def monstAbilities(self):
        ...
        self.abilities = ['Nightmarish','Overwhelming','Special','Ambush','Mask','Undead','Elusive']
    ...
    def getData(self):
        ...
        for i in range(len(self.mvType)):
            if(self.mv_RB[i].isChecked()):
                ...
                if(self.mvType[i] == 'Special'):
                    monsterAttributes["movement"] = self.mv_specBox.text()
                else:
        ...
        for i in range(len(self.dim_RB)):
            if(self.dim_RB[i].isChecked()):
                monsterAttributes["symbol"] = self.dimImages[i][:-4]
                break
        ...
        for i in range(len(self.abil_cb)):
            if(self.abilities[i] == 'Nightmarish' and self.abil_cb[i].isChecked()):
                    monsterAttributes[self.abilities[i].lower()] = int(self.nightBox.text())
            elif(self.abilities[i] == 'Overwhelming' and self.abil_cb[i].isChecked()):
                    monsterAttributes[self.abilities[i].lower()] =  int(self.overBox.text())
            elif(self.abilities[i] == 'Special' and self.abil_cb[i].isChecked()):
                    monsterAttributes[self.abilities[i].lower()] = str(self.abil_specBox.text())
            else:
                    monsterAttributes[self.abilities[i].lower()] =  int(self.abil_cb[i].isChecked())

        shortRes = ['mag_res','mag_imm','phys_res','phys_imm','weapon_imm']
        
        for i in range(len(self.res_cb)):
            monsterAttributes[shortRes[i]] = int(self.res_cb[i].isChecked())
        ...
        monsterAttributes["sneak_mod"] = int(self.evasion.text())
        monsterAttributes["horror_mod"] = int(self.horMod.text())
        monsterAttributes["horror_dmg"] = int(self.horDmg.text())
        monsterAttributes["combat_mod"] = int(self.comMod.text())
        monsterAttributes["combat_dmg"] = int(self.comDmg.text())
        monsterAttributes["toughness"] = int(self.toughness.text())
        ...
        #self.database.submitData(monsterAttributes)
